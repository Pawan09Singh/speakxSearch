  ...
  return (
    <>
      ...
      <div className="result-count">
        <p>
          {(20 * (+currentPage - 1)) + 1} - {(20 * (+currentPage - 1)) + 20} of over{' '}
          {20 * totalPages} questions for{' '}
        ...
        <div className="question-lists" ref={questionTypeRef}>
          {queryBasedQuestion &&
            ...
            queryBasedQuestion.map((question, index) => (
              <div
                className="question-with-type"
                key={index}
                onClick={() => scrollToQuestion(index - 1)}
              >
                <div className="question-details">
                  <div className="question-text">
                    Question {(20 * (+currentPage - 1)) + index + 1}
                  </div>
          ...
          {queryBasedQuestion && queryBasedQuestion.length > 0 ? (
            queryBasedQuestion.map((question, index) => (
              <QuestionUi
                ...
                index={index}
                key={index}
                questionNo={(20 * (+currentPage - 1)) + index}
              />
            ))
          ) : (
